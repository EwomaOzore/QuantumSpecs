...
export function AnalyticsClient({
  ...
  const [metric, setMetric] = useState<
    "volume" | "revenue" | "failureRate" | "avgLatencyMs"
  >("volume");
      ...
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-[13px] text-qs-muted">
            Checkout, FX corridors and provider reliability across Kora’s five
            markets.
          </p>
        </div>
      ...
      <div className="mt-4 rounded-lg border border-qs-border bg-qs-surface">
        <div className="flex flex-wrap items-center justify-between gap-2 border-b border-qs-border px-4 py-2">
          <div className="text-[13px] text-qs-muted">
            ...
            {formatPercent(initial.failureRate * 100)} fail ·{" "}
            {Math.round(initial.avgLatencyMs)}ms
          </div>
          <div className="flex gap-1">
            {(
              ["volume", "revenue", "failureRate", "avgLatencyMs"] as const
            ).map((m) => (
              <button
                key={m}
                type="button"
            ...
            {initial.byChannel.map((c) => (
              ...
              >
                ...
                <span className="font-mono text-qs-muted tabular">
                  {formatInt(c.count)} ·{" "}
                  {formatPercent((c.count ? c.failed / c.count : 0) * 100)} fail
                </span>
              </div>
            ))}
          </div>
