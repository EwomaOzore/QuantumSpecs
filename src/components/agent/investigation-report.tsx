...
export function InvestigationReport({
  ...
  return (
    <div>
      <p className="text-[15px] leading-7 text-qs-text">
        {investigation.summary}
      </p>
      <div className="mt-4 flex flex-wrap gap-x-4 gap-y-1 text-[11px] uppercase tracking-[0.12em] text-qs-faint">
          ...
          {investigation.evidence.map((item) => (
            ...
            >
              <div>
                ...
                <div className="mt-0.5 text-[12px] leading-5 text-qs-muted">
                  {item.detail}
                </div>
              </div>
              {item.metric ? (
                <div className="shrink-0 font-mono text-[13px] tabular text-qs-accent">
                  {item.metric}
                </div>
              ) : null}
            </div>
