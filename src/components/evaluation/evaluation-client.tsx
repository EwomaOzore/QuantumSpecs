...
export function EvaluationClient() {
    ...
    queryFn: async () => {
      ...
      return {
        ...
        cases: payload.cases.map((c) => ({
          ...c,
          toolsUsed:
            c.toolsUsed ?? (c.toolCallsJson ? JSON.parse(c.toolCallsJson) : []),
        })),
      } as EvalPayload;
    },
  });
  ...
  return (
    <div className="px-4 py-4 sm:px-6 sm:py-5">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-[13px] text-qs-muted">
            Scores the built-in local analyst on tool choice, evidence, and
            latency. Paid models stay off.
          </p>
        </div>
        <Button
          variant="primary"
      ...
      {status ? (
        <p className="mt-3 text-[13px] text-qs-accent">{status}</p>
      ) : null}
      ...
      {data && cases.length ? (
        <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
          ...
          <Card className="px-4 py-3">
            ...
            <div className="mt-1 font-mono text-[22px]">
              {cases.length
                ? Math.round(
                    cases.reduce((s, c) => s + c.latencyMs, 0) / cases.length,
                  )
                : 0}
              ms
            </div>
        ...
        <p className="mt-6 text-[13px] text-qs-muted">
          Run the suite to score the operations analyst against Kora fixtures.
          Each case calls the live agent.
        </p>
      )}
              ...
              <tbody className="divide-y divide-qs-border">
                {cases.map((c) => (
                  <tr key={c.caseId}>
                    ...
                    <td className="px-4 py-2.5 text-[12px] text-qs-muted">
                      {c.notes}
                    </td>
                  </tr>
                ))}
      ...
      <div className="mt-4 grid gap-3 lg:grid-cols-3">
        <Card className="px-4 py-3">
          ...
          <p className="mt-1 text-[12px] leading-5 text-qs-muted">
            Evidence must quote tool output. Empty evidence after a successful
            tool call fails the case.
          </p>
        </Card>
        <Card className="px-4 py-3">
        ...
        <Card className="px-4 py-3">
          ...
          <p className="mt-1 text-[12px] leading-5 text-qs-muted">
            The suite calls the same local analyst as the console. OpenAI is not
            contacted, so there is no model bill.
          </p>
        </Card>
      </div>
    </div>
